(ns tracky.infrastructure.xtdb.settings
  (:require [integrant.core :as ig]
            ...
            [tracky.infrastructure.xtdb.client :as client]))

(defn save! [user-id {:keys [toggl-api-key tempo-api-key jira-account-id]}]
  (client/put [{:xt/id user-id
...
(defn fetch! [user-id]
  (ffirst (client/query '[:find (pull ?e [:settings/toggl-api-key :settings/tempo-api-key :settings/jira-account-id])
                          :in $ ?user-id
